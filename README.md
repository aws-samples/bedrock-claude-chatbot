...
* Run command `python3 -m streamlit run bedrock-chat.py --server.enableXsrfProtection false` to start the Streamlit server. The reason for **--server.enableXsrfProtection false** is to enable document upload, if assigned to `true` you would not be able to upload documents when the app is hosted in Sagemaker Studio, however, the rest of the app functionality will work. Do not use the links generated by the command as they won't work in studio.
...
* Copy the **External URL** link generated and paste in a new browser tab. ⚠️ **Note:** This link is not SSL/TLS encrypted, I recommended to configure HTTPS by bringing your own domain name and SSL/TLS certificate.
